...
import { optionConditions } from '../../constants';
...
export const SlideInformation13 = () => {
  ...
  const {
    ...
    defaultValues: {
      checkConditions: formData.checkConditions || [],
    },
  });
    ...
    <div className='flex flex-row items-center justify-center w-4/5 gap-4 md:py-10 py-20 md:pr-15 pr-0 md:h-screen min-h-screen max-h-fit md:overflow-y-scroll  custom-scrollbar'>
      <div className='flex flex-col items-start justify-start md:text-justify text-start    gap-4 w-full'>
        <QuestionRimac className=' w-full'>
          <QuestionRimac.Label
            size={textLabel}
            text='¿Te has hospitalizado por alguna de las siguientes condiciones médicas?'
          ></QuestionRimac.Label>
          <MultiCheckboxForm
            {...{ control }}
            name='checkConditions'
            options={optionConditions}
            message={errors?.checkConditions?.message}
          />
        </QuestionRimac>
