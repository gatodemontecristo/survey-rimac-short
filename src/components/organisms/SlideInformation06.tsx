...
import { optionCigarettes } from '../../constants';

const schema = yup.object().shape({
  smoke: yup.string().required('Debes seleccionar una opción'),
});
    ...
    <div className='flex flex-row items-center justify-start w-4/5 gap-4 py-20 md:py-10  md:h-screen  min-h-[85vh] max-h-fit md:overflow-y-scroll custom-scrollbar'>
      <div className='flex flex-col items-start justify-start md:text-justify text-start   gap-4 w-full'>
        <QuestionRimac className='mb-4'>
          <QuestionRimac.Label
            size={textLabel}
            text='¿Con qué frecuencia fumas?'
          ></QuestionRimac.Label>

          <RadioCollection
            {...{ control }}
            name='smoke'
            itemOptions={optionCigarettes}
            message={
              typeof errors?.smoke?.message === 'string'
